  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="relative pt-32 pb-16">
        ...
        <div className="max-w-7xl mx-auto px-6 relative">
          <div className="max-w-3xl mx-auto text-center mb-12">
            <Badge className="mb-6 px-4 py-2 bg-muted border-border text-white backdrop-blur-sm">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="relative group">
              ...
              <div className="relative flex items-center bg-muted backdrop-blur-xl rounded-2xl border border-border p-2">
                <Search className="w-5 h-5 text-muted-foreground ml-4" />
                <Input
                  placeholder="Search events, categories, locations..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="flex-1 bg-transparent border-0 text-white placeholder:text-muted-foreground focus-visible:ring-0 text-lg h-12"
                />
                {searchQuery && (
                  <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-white mr-2" onClick={() => setSearchQuery('')}>
                    <X className="h-4 w-4" />
                  </Button>
                )}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-8">
            ...
            <Button
              ...
              className={cn(
                ...
                selectedCategory === category 
                  ...
                  : "bg-muted/40 text-muted-foreground hover:bg-muted hover:text-white border border-border"
              )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-b border-border mb-8">
          <div className="flex items-center gap-3">
            <Select value={dateFilter} onValueChange={setDateFilter}>
              <SelectTrigger className="w-[140px] bg-muted/40 border-border text-white rounded-xl">
              ...
              <SelectContent className="bg-background/90 border-border">
                {dateFilters.map((filter) => (
                  <SelectItem key={filter.value} value={filter.value} className="text-muted-foreground">{filter.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>

            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="w-[160px] bg-muted/40 border-border text-white rounded-xl">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-background/90 border-border">
                {sortOptions.map((option) => (
                  <SelectItem key={option.value} value={option.value} className="text-muted-foreground">{option.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="outline" className="bg-muted/40 border-border text-white rounded-xl">
                  <SlidersHorizontal className="h-4 w-4 mr-2" /> Filters
                  {activeFiltersCount > 0 && <Badge className="ml-1 bg-purple-500">{activeFiltersCount}</Badge>}
                </Button>
              </SheetTrigger>
              <SheetContent className="bg-background/95 border-border text-white">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-muted-foreground">{events.length} events loaded</span>
            <div className="flex items-center bg-muted/40 border border-border rounded-xl p-1">
              <Button
                variant="ghost"
                size="icon"
                className={cn("h-8 w-8", viewMode === 'grid' ? "bg-muted text-white" : "text-muted-foreground")}
              ...
              <Button
                variant="ghost"
                size="icon"
                className={cn("h-8 w-8", viewMode === 'list' ? "bg-muted text-white" : "text-muted-foreground")}
              ...
              <div className="mb-10">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold flex items-center gap-3 text-white">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-primary flex items-center justify-center"><Brain className="h-5 w-5 text-white" /></div>
                    Recommended For You
                  </h2>
                ...
                {aiLoading ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[1, 2, 3, 4].map((i) => <div key={i} className="h-[300px] rounded-2xl bg-muted/40 animate-pulse" />)}
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {aiRecommendations.slice(0, 4).map((rec) => {
                      ...
                      return <EventCard key={event.id} event={event} variant="default" />;
                        ...
                        {displayedEvents.length === 0 && status !== "LoadingFirstPage" ? (
                          ...
                          <div className="flex flex-col items-center justify-center py-20 bg-muted/40 border border-border rounded-3xl">

                            <Search className="h-16 w-16 text-muted-foreground mb-6" />

                            <h3 className="text-2xl font-bold text-white mb-2">No events found</h3>

                            <Button variant="outline" className="border-border text-white mt-6" onClick={clearFilters}>Clear Filters</Button>
