    ...
    <div className="space-y-4">
      {!recommendations ? (
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
          <div className="bg-primary/10 border border-cyan-500/20 p-4 rounded-xl">
            <p className="text-xs text-cyan-200 font-medium mb-2 flex items-center gap-2">
              <Sparkles className="h-3 w-3" /> AI Strategy
            </p>
            <p className="text-xs text-foreground/80 italic">{strategy}</p>
          </div>

          <div className="grid gap-3">
            {recommendations.map((rec, i) => (
              <Card key={i} className="bg-card border-border overflow-hidden hover:border-primary/30 transition-all cursor-default">
                <CardContent className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-primary" />
                      <span className="font-bold text-sm text-foreground">{rec.dayOfWeek}</span>
                    </div>
                    <Badge className="bg-primary/20 text-primary-foreground text-[10px]">{rec.score}% Fit</Badge>
                  </div>
