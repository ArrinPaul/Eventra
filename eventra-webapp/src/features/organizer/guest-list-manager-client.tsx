      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="bg-card border-border text-foreground">
          ...
          <CardContent className="space-y-6">
             <div className="p-6 border-2 border-dashed border-border rounded-xl bg-card text-center space-y-4">
             ...
             <div className="space-y-2">
                ...
                <select 
                  className="w-full bg-card border border-border rounded-md p-2 text-sm focus:ring-1 focus:ring-emerald-500 outline-none"
                  value={selectedTierId}
                  onChange={e => setSelectedTierId(e.target.value)}
                >
                  <option value="" className="bg-card">No Tier / General</option>
                  {ticketTiers.map(tier => (
                    <option key={tier.id} value={tier.id} className="bg-card">{tier.name} (${tier.price})</option>
                  ))}
                </select>
             </div>
             ...
             <div className="flex gap-4">
                ...
                <Button 
                  className="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-foreground" 
             ...
             {importResults ? (
               <div className="p-6 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                     ...
                     <div className="p-4 rounded-lg bg-destructive/10 border border-red-500/20 text-center">
                        <p className="text-xs text-muted-foreground uppercase font-black mb-1">Failed</p>
                        <p className="text-3xl font-black text-destructive">{importResults.failed}</p>
                     </div>
                  </div>
                  ...
                  {importResults.errors.length > 0 && (
                    <div className="space-y-2">
                       ...
                       <div className="max-h-48 overflow-y-auto rounded-lg bg-background/40 border border-border/50 p-3 font-mono text-[10px] text-destructive">
                ...
                <div className="divide-y divide-white/5">
                   {previewData.map((row, i) => (
                     <div key={i} className="px-6 py-3 flex justify-between items-center hover:bg-card transition-colors">
                        <div>
