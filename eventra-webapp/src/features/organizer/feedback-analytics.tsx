    ...
    <div className="space-y-6 text-white">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            ...
            <div className="flex items-center gap-1 mt-2">
              ...
              <span className="text-[10px] text-gray-500 ml-2">from {analytics.total} reviews</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2 bg-cyan-500/10 rounded-lg"><TrendingUp className="text-cyan-400" /></div>
              <Badge className="bg-cyan-500/20 text-cyan-400 border-0">NPS Score</Badge>
            </div>
        ...
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            ...
            <p className="text-[10px] text-gray-500 mt-2">Total feedback submissions</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="bg-white/5 border-white/10 text-white">
          <CardHeader>
            <CardTitle className="text-sm font-bold uppercase tracking-widest text-gray-500">Rating Distribution</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {ratingDistribution.map((item) => (
              <div key={item.star} className="flex items-center gap-4">
                ...
                <span className="text-xs text-gray-500 w-10 text-right">{Math.round(item.percentage)}%</span>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/5 border-white/10 text-white overflow-hidden flex flex-col">
          <CardHeader>
            <CardTitle className="text-sm font-bold uppercase tracking-widest text-gray-500">Attendee Sentiment</CardTitle>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col justify-center gap-8">
            <div className="flex justify-around items-center text-center">
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-500 uppercase font-black">Promoters</p>
              </div>
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-500 uppercase font-black">Passives</p>
              </div>
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-500 uppercase font-black">Detractors</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-white/5 border-white/10 text-white">
        <CardHeader>
          <CardTitle className="text-sm font-bold uppercase tracking-widest text-gray-500">Recent Comments</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {analytics.feedbackList.slice(0, 5).map((f, i) => (
              <div key={i} className="p-4 bg-white/5 rounded-lg border border-white/5 flex gap-4">
                ...
                <div>
                  <p className="text-sm italic text-gray-300">"{f.comment || 'No comment provided'}"</p>
                  <p className="text-[10px] text-gray-500 mt-2">{new Date(f.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
            {analytics.total === 0 && (
              <div className="text-center py-10 text-gray-500 italic">No comments yet.</div>
            )}
          </div>
        </CardContent>
      </Card>
