      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="explore" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.filter((e: any) => e.status === 'published').map((event: any) => (
            <Card key={event._id} className="bg-muted/40 border-border text-white overflow-hidden flex flex-col">
              <div className="h-40 bg-gradient-to-br from-primary/40 to-purple-900/40 relative">
              ...
              <CardContent className="flex-1 space-y-4">
                ...
                <div className="pt-4 border-t border-border flex items-center justify-between">
                  <p className="text-2xl font-bold text-primary">{event.isPaid ? `$${event.price}` : 'FREE'}</p>
        ...
        <TabsContent value="my-tickets" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {myTicketsRaw.map((ticket: any) => (
            <Card key={ticket._id} className="bg-muted/40 border-border text-white overflow-hidden relative">
