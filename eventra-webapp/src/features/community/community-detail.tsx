  ...
  return (
    <div className="container mx-auto px-4 py-6 text-white space-y-6">
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-bold text-2xl border border-primary/20">
                {community.name.charAt(0)}
              </div>
              <div>
                <CardTitle className="text-3xl font-bold">{community.name}</CardTitle>
                <CardDescription className="text-muted-foreground mt-1">{community.category} Community</CardDescription>
              </div>
            </div>
            <div className="flex gap-2">
                {isMember ? (
                    <>
                        <Button onClick={() => setShowCreatePost(true)} className="bg-primary hover:bg-primary"><Plus className="mr-2 h-4 w-4" /> New Post</Button>
                    ...
                    <Button onClick={handleJoin} className="bg-primary hover:bg-primary">Join Community</Button>
                )}
            </div>
          </div>
          <div className="flex gap-6 mt-4 pt-4 border-t border-border text-sm text-muted-foreground">
            <span className="flex items-center gap-1"><Users size={14} className="text-primary" /> {community.membersCount} members</span>
      ...
      <Tabs defaultValue="feed" className="space-y-6">
        <TabsList className="bg-muted/40 border-border p-1">
          <TabsTrigger value="feed" className="data-[state=active]:bg-primary">Feed</TabsTrigger>
        ...
        <TabsContent value="feed" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
                {posts.map((p: any) => (
                <Card key={p._id} className="bg-muted/40 border-border text-white overflow-hidden">
                    <CardHeader className="p-4 flex flex-row items-center gap-3">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={p.authorImage} />
                        <AvatarFallback className="bg-primary/10 text-primary text-[10px]">{p.authorName?.charAt(0)}</AvatarFallback>
                      </Avatar>
                    ...
                    <CardContent className="p-4 pt-0">
                        ...
                        <div className="flex items-center gap-4 mt-6 pt-4 border-t border-border/60">
                          <Button 
                            variant="ghost" 
                            size="sm" 
                            className={p.meLiked ? "text-primary" : "text-muted-foreground"}
                ...
                {posts.length === 0 && postsStatus !== "LoadingFirstPage" && (
                  <div className="py-20 text-center text-muted-foreground border border-dashed border-border rounded-lg italic">
                ...
                {postsStatus === "CanLoadMore" && (
                  <div className="flex justify-center pt-4">
                    <Button variant="outline" onClick={() => loadMorePosts(10)} className="border-border">Load More Posts</Button>
                  </div>
                )}

                {(postsStatus === "LoadingMore" || postsStatus === "LoadingFirstPage") && (
                  <div className="flex justify-center pt-4"><Loader2 className="animate-spin text-primary" /></div>
                )}
            </div>
          ...
          <Card className="bg-muted/40 border-border text-white">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {members.map((m: any) => (
                  <div key={m._id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/40 border border-border">
                    <Avatar>
                      <AvatarImage src={m.image} />
                      <AvatarFallback className="bg-primary/10 text-primary">{m.name?.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className="overflow-hidden">
                      <p className="text-sm font-bold truncate">{m.name}</p>
                      <p className="text-[10px] text-primary uppercase tracking-wider font-mono">{m.role}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {membersStatus === "CanLoadMore" && (
                <div className="flex justify-center mt-6">
                  <Button variant="outline" onClick={() => loadMoreMembers(20)} className="border-border">Load More Members</Button>
                </div>
              )}

              {(membersStatus === "LoadingMore" || membersStatus === "LoadingFirstPage") && (
                <div className="flex justify-center mt-6"><Loader2 className="animate-spin text-primary" /></div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-border">
                   <div>
                     ...
                     <p className="text-sm text-muted-foreground">Be respectful, stay on topic, and help grow the community. All posts are subject to AI moderation for platform safety.</p>
                   </div>
                   <div>
                     <h4 className="font-bold mb-2">Moderation</h4>
                     <p className="text-sm text-muted-foreground">This community is moderated by its creator and automated AI tools. Violating content will be flagged and removed.</p>
                   </div>
                </div>
              </CardContent>
           </Card>
      ...
      <Dialog open={showCreatePost} onOpenChange={setShowCreatePost}>
        <DialogContent className="bg-gray-900 border-border text-white">
          ...
          <Textarea 
            ...
            className="bg-muted/40 border-border min-h-[150px]" 
          />
          <DialogFooter>
