  ...
  return (
    <div className="space-y-8 text-foreground max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Events Export */}
        <Card className="bg-card border-border text-foreground overflow-hidden group hover:border-primary/30 transition-all">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <Button variant="outline" size="sm" onClick={() => exportEvents('csv')} disabled={exporting === 'events' || myEvents.length === 0} className="border-border hover:bg-card">
                <TableIcon className="w-4 h-4 mr-2" /> CSV
              </Button>
              <Button variant="outline" size="sm" onClick={() => exportEvents('json')} disabled={exporting === 'events' || myEvents.length === 0} className="border-border hover:bg-card">
        ...
        {/* Tickets Export */}
        <Card className="bg-card border-border text-foreground overflow-hidden group hover:border-purple-500/30 transition-all">
          <CardHeader className="pb-4">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <Button variant="outline" size="sm" onClick={() => exportTickets('csv')} disabled={exporting === 'tickets' || myTickets.length === 0} className="border-border hover:bg-card">
                <TableIcon className="w-4 h-4 mr-2" /> CSV
              </Button>
              <Button variant="outline" size="sm" onClick={() => exportTickets('json')} disabled={exporting === 'tickets' || myTickets.length === 0} className="border-border hover:bg-card">
                <FileJson className="w-4 h-4 mr-2" /> JSON
              </Button>
            </div>
          </CardContent>
