  ...
  return (
    <div className="space-y-8 text-white">
      <div>
        ...
        <p className="text-gray-400">Insights and attendee satisfaction for <span className="text-white font-bold">{eventTitle}</span></p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <p className="text-[10px] uppercase font-black text-gray-500 tracking-widest">Average Rating</p>
        ...
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <p className="text-[10px] uppercase font-black text-gray-500 tracking-widest">Net Promoter Score</p>
        ...
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <p className="text-[10px] uppercase font-black text-gray-500 tracking-widest">Total Responses</p>
        ...
        <Card className="bg-white/5 border-white/10 text-white">
          <CardContent className="p-6">
             <div className="flex justify-between items-start mb-4">
                <p className="text-[10px] uppercase font-black text-gray-500 tracking-widest">Sentiment</p>
        ...
        <CardContent>
           <div className="space-y-4">
              {feedbackList.map((feedback) => (
                <div key={feedback.id} className="p-4 rounded-xl bg-white/5 border border-white/5 space-y-3">
                   ...
                   {feedback.comment && (
                     <p className="text-sm text-gray-300 italic">"{feedback.comment}"</p>
                   )}
                   {feedback.responses && Object.keys(feedback.responses).length > 0 && (
                     <div className="pt-2 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-2">
                        {Object.entries(feedback.responses).map(([qId, val]: [string, any]) => (
                          <div key={qId} className="text-[10px]">
                             <span className="text-gray-500">Q-{qId}:</span> <span className="text-cyan-500">{val}</span>
                          </div>
                        ))}
                     </div>
                   )}
