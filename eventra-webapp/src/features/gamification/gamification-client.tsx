...
export function GamificationClient() {
  ...
  return (
    <div className="container mx-auto px-4 py-6 text-foreground space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <Trophy className="text-warning" /> Gamification Hub
          </h1>
          <p className="text-muted-foreground mt-2">Track your progress and earn rewards</p>
        </div>
        <div className="flex gap-4">
          <Card className="bg-card border-border text-foreground min-w-[150px]">
            <CardContent className="p-4 flex items-center gap-3">
              <Zap className="text-primary" size={24} />
              <div>
                <p className="text-[10px] text-muted-foreground uppercase">Current XP</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="bg-card border-border text-foreground md:col-span-2">
              <CardHeader>
                ...
                <CardDescription className="text-muted-foreground">Your latest achievements and points</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {pointsHistory.slice(0, 5).map((entry: any) => (
                    <div key={entry.id} className="flex items-center justify-between p-3 bg-card rounded-lg border border-border">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-primary/10 rounded-full"><Zap size={16} className="text-primary" /></div>
                        <div>
                          <p className="text-sm font-medium">{entry.reason}</p>
                          <p className="text-[10px] text-muted-foreground">{formatDistanceToNow(entry.createdAt, { addSuffix: true })}</p>
                        </div>
                      </div>
                      <p className="text-primary font-bold">+{entry.points}</p>
                    </div>
                  ))}
                  {pointsHistory.length === 0 && <p className="text-center py-10 text-muted-foreground italic">No activity yet. Start attending events to earn points!</p>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card border-border text-foreground">
              ...
              <CardContent className="flex flex-col items-center py-6">
                ...
                <p className="mt-6 text-sm text-muted-foreground">Next level at {((user?.level || 1) * 1000)} XP</p>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="badges">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {badgeDefinitions.map((badge: any) => {
              ...
              return (
                <Card key={badge.id} className={cn("bg-card border-border text-foreground transition-all hover:scale-105", !isEarned && "opacity-40 grayscale")}>
                  <CardContent className="p-6 flex flex-col items-center text-center gap-3">
                    ...
                    {isEarned && <Badge className="bg-warning/20 text-warning border-yellow-500/20 text-[8px] uppercase">Earned</Badge>}
                  </CardContent>
                </Card>
              );
            })}
            {badgeDefinitions.length === 0 && <div className="col-span-full py-20 text-center text-muted-foreground border border-dashed border-border rounded-lg">No badges defined yet.</div>}
          </div>
        </TabsContent>
              ...
              <div className="divide-y divide-white/10">
                {pointsHistory.map((entry: any) => (
                  <div key={entry.id} className="p-4 flex items-center justify-between">
                    ...
                    <p className="text-lg font-bold text-primary">+{entry.points} XP</p>
                  </div>
                ))}
              </div>
            </CardContent>
