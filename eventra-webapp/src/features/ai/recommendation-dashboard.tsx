...
export default function AiRecommendationDashboard() {
  ...
  const getConfidenceBadgeColor = (level: string) => {
    switch (level) {
      case 'high': return 'bg-success/20 text-success border-green-500/20';
      case 'medium': return 'bg-primary/20 text-primary border-cyan-500/20';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {insights.map((insight, index) => (
          <Card key={index} className="bg-card border-border overflow-hidden group hover:border-primary/30 transition-colors">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm text-muted-foreground">{insight.title}</p>
                <div className="p-2 bg-card rounded-lg text-primary group-hover:scale-110 transition-transform">
                  {insight.icon}
                </div>
              </div>
      ...
      <Card className="bg-card/60 border-border backdrop-blur-md overflow-hidden">
        ...
        <CardContent className="p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="bg-card border border-border p-1 mb-8">
              <TabsTrigger value="events" className="data-[state=active]:bg-primary data-[state=active]:text-foreground">
                <Calendar className="w-4 h-4 mr-2" />
                Events
              </TabsTrigger>
              <TabsTrigger value="content" className="data-[state=active]:bg-primary data-[state=active]:text-foreground">
            ...
            <TabsContent value="events" className="space-y-6">
              {eventRecommendations.map((event) => (
                <Card key={event.eventId} className="bg-card border-border hover:border-primary/50 transition-all group overflow-hidden">
                  <CardContent className="p-6">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                      <div className="flex-1 space-y-4">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="font-bold text-xl text-foreground group-hover:text-primary transition-colors">{event.title}</h3>
                        ...
                        <p className="text-foreground/80 italic">"{event.personalizedPitch}"</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {event.preparationSuggestions.map((s, i) => <Badge key={i} variant="secondary" className="bg-card text-[10px]">{s}</Badge>)}
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start w-full md:w-auto gap-4">
                        ...
                        <Button className="bg-primary hover:bg-primary/90 text-foreground shadow-lg shadow-cyan-900/20">
                          Register Now
                        </Button>
                      </div>
                    </div>
            ...
            <TabsContent value="content" className="space-y-6">
              {contentRecommendations.map((content) => (
                <Card key={content.contentId} className="bg-card border-border hover:border-primary/50 transition-all overflow-hidden">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge variant="outline" className="bg-card text-muted-foreground border-border/50">{content.difficulty}</Badge>
                        </div>
                    ...
                    <div className="flex justify-between items-center mt-6 pt-4 border-t border-border/50">
                      ...
                      <Button size="sm" className="bg-muted hover:bg-white/20 text-foreground border-0">
                        Start Learning
                      </Button>
                    </div>
                  </CardContent>
            ...
            <TabsContent value="connections" className="space-y-6">
              {connectionRecommendations.map((connection) => (
                <Card key={connection.userId} className="bg-card border-border hover:border-primary/50 transition-all overflow-hidden">
                  <CardContent className="p-6">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2">
                            <h3 className="font-bold text-lg text-foreground">{connection.name}</h3>
                    ...
                    <div className="mt-6 space-y-4 bg-card rounded-xl p-4 border border-border/50">
                      ...
                      <div className="space-y-2">
                        {connection.conversationStarters.map((starter: string, idx: number) => (
                          <div key={idx} className="bg-card p-2 rounded-lg text-xs text-foreground/80 border border-border/50">
                            "{starter}"
                          </div>
                        ))}
                      </div>
