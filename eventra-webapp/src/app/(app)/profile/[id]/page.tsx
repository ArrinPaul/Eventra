...
export default function UserProfilePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl text-foreground space-y-8 animate-in fade-in duration-700">
      {/* Profile Header */}
      <Card className="bg-[#050505] border-border text-foreground overflow-hidden rounded-[2rem] shadow-2xl relative">
        ...
        <CardContent className="p-8 -mt-20 relative z-10">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-6">
            <div className="relative group">
               ...
               <Avatar className="h-32 w-32 border-4 border-[#050505] shadow-2xl relative z-10">
                ...
                <AvatarFallback className="bg-gradient-to-br from-cyan-500/20 to-purple-500/20 text-primary text-4xl font-black">
            ...
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-3">
                 ...
                 {profileUser.role === 'admin' && <Shield size={20} className="text-primary fill-cyan-400/10" />}
              </div>
          ...
          {profileUser.bio && (
            <p className="mt-8 text-muted-foreground text-sm leading-relaxed max-w-2xl border-l-2 border-cyan-500/20 pl-4 italic">
              {profileUser.bio}
            </p>
          )}
          ...
          {stats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10">
              <div className="p-4 bg-card rounded-2xl border border-border/50 hover:border-primary/30 transition-all group">
                ...
                <div className="flex items-baseline gap-2">
                   <p className="text-3xl font-black text-foreground">{stats.level}</p>
                   <p className="text-[10px] text-muted-foreground">Tier {Math.ceil(stats.level / 10)}</p>
                </div>
                <div className="w-full h-1 bg-card rounded-full mt-3 overflow-hidden">
                   <div className="h-full bg-cyan-500" style={{ width: `${(stats.xp % 100)}%` }} />
                </div>
              </div>
              
              <div className="p-4 bg-card rounded-2xl border border-border/50 hover:border-purple-500/30 transition-all group">
                ...
                <div className="flex items-center gap-2">
                   ...
                   <p className="text-3xl font-black text-foreground">{stats.xp.toLocaleString()}</p>
                </div>
              </div>

              <div className="p-4 bg-card rounded-2xl border border-border/50 hover:border-amber-500/30 transition-all group">
                ...
                <div className="flex items-center gap-2">
                   ...
                   <p className="text-3xl font-black text-foreground">{stats.badgeCount}</p>
                </div>
              </div>

              <div className="p-4 bg-card rounded-2xl border border-border/50 hover:border-emerald-500/30 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <Card className="bg-card border-border text-foreground h-fit">
            ...
            <CardContent className="space-y-6">
               ...
               <Button variant="outline" className="w-full border-border text-xs font-bold uppercase tracking-widest h-10 hover:bg-card" asChild>
