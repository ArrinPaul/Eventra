  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(`/calendar?error=${encodeURIComponent(errorDescription)}`, request.url)
      );
    }
    ...
    if (state) {
      try {
        ...
        returnUrl = stateData.returnUrl || '/calendar';
      } catch {
    ...
    const tempConnectionId = `cal_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    if (adminDb) {
      await adminDb.collection('google_calendar_connections').doc(tempConnectionId).set({
        ...connectionData,
        claimed: false,
        createdAt: new Date(),
    ...
    // Redirect back to calendar page with success indicator
    return NextResponse.redirect(
      new URL(
        `${returnUrl}?calendar_connected=true&connection_id=${tempConnectionId}&email=${encodeURIComponent(userInfo.email)}`,
        request.url
      )
    );
  } catch (error) {
