...
import { getCalendarEvents } from '@/app/actions/events';
...
export default function CalendarPage() {
  ...
  return (
    <div className="container py-8 text-foreground">
      <div className="mb-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        hover:bg-muted
                        ${isSelected ? 'bg-primary/20 ring-1 ring-cyan-500' : ''}
                        ${today ? 'border border-cyan-500/40' : ''}
                        ${!isSameMonth(day, currentMonth) ? 'opacity-30' : ''}
                      `}
                    >
                      <span className={`text-sm ${today ? 'text-primary font-bold' : 'text-foreground/80'}`}>
                        {format(day, 'd')}
                      </span>
                      {dayEvents.length > 0 && (
        ...
        {/* Selected Day Events */}
        <div>
          <Card className="bg-card border-border text-foreground sticky top-8">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              {selectedEvents.map((event: EventraEvent) => (
                <Link href={`/events/${event.id}`} key={event.id}>
                  <div className="p-4 rounded-xl bg-card border border-border hover:border-primary/50 transition-all cursor-pointer">
