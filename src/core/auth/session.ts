...
function getSessionSecret(): string {
  const configured = process.env.JWT_SECRET || process.env.AUTH_SECRET || process.env.SESSION_SECRET;
  if (configured) return configured;
  if (process.env.NODE_ENV === 'production') {
    throw new Error('JWT_SECRET, AUTH_SECRET, or SESSION_SECRET is required in production');
  }
