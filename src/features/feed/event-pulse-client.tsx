...
export function EventPulseClient({ eventId, initialStats }: PulseProps) {
  ...
  return (
    <div className="space-y-6 text-foreground">
        ...
        <Card className="bg-card border-border text-foreground overflow-hidden relative group">
          ...
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              ...
              <Progress value={registrationRate} className="h-1 bg-card" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border text-foreground overflow-hidden relative group">
          ...
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              ...
              <Progress value={checkInRate} className="h-1 bg-card" />
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-background/20 border-border text-foreground">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="bg-card border-border text-[10px]">Last 10 updates</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="h-32 w-full flex items-end gap-1 px-2">
            {history.map((h, i) => (
              <div 
                key={i} 
                className="flex-1 bg-cyan-500/40 rounded-t hover:bg-primary/90 transition-all group relative"
                style={{ height: `${(h / stats.capacity) * 100}%`, minHeight: '4px' }}
              >
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-card border border-border text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                  {h} Reg
                </div>
              </div>
            ))}
