...
export default function OrganizerDashboard() {
  ...
  const organizerEvents = events.filter((e: any) => e.organizerId === (user?._id || user?.id));
  const filteredEvents = organizerEvents.filter((e: any) => 
    e.title.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const totalRegistrations = organizerEvents.reduce((sum: number, e: any) => sum + (e.registeredCount || 0), 0);
  const activeEvents = organizerEvents.filter((e: any) => e.status === 'published').length;
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription className="text-gray-400">You have created {organizerEvents.length} events</CardDescription>
            </div>
