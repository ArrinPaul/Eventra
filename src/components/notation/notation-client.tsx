...
const NotationClient: React.FC<NotationClientProps> = ({ eventId, sessionId, userId }) => {
  ...
  const handleCreateNotation = async () => {
    ...
    setSaving(true);
    try {
      const result = await createNotation({
        eventId,
        sessionId,
        title: title.trim(),
        content,
  ...
  const handleUpdateNotation = async () => {
    ...
    try {
      await updateNotation({
        notationId: selectedNotation.id,
        title: title.trim(),
        content
      });
      ...
      toast.error(error.message || 'Failed to update notation');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleShareNotation = async () => {
    ...
    try {
      ...
      toast.success('Notation shared successfully');
      setShareDialogOpen(false);
      setCollaboratorEmails('');
    } catch (error: any) {
      console.error('Error sharing notation:', error);
      toast.error(error.message || 'Failed to share notation');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleGenerateAISummary = async (notationId: string) => {
    setSaving(true);
    try {
      const result = await generateAISummary({ notationId });
